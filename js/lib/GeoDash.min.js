...
GeoDash.version = "0.1-dev", GeoDash.noConflict = function() {
    ...
    makeTitle: function() {
        if (this.options.title) {
            var t = '<h5>';
            t += this.options.title, t += "</h5>", d3.select(this.el).html(t);
        }
    }
    ...
    drawChart: function() {
        ...
        this.options.title && (this.height = this.height - 30), this.formatPercent = d3.format(".0%"), 
        ...
        console.log(this.options.width), this.xAxisElement.append("rect").attr("width", this.width).attr("height", this.height).attr("transform", "translate(0,1)").style("fill", i);
    },
    update: function(t) {
    ...
    update: function(t) {
        var i = this, e = this.tbody.selectAll("tr").data(t).attr("style", function(t) {
            for (var e = t[i.options.columns[0]], s = 0; s < i.options.highlight.length; s++) if (i.options.highlight[s] == e) return "background:" + i.options.color;
            return "";
        });
    ...
    format: function(t) {
        var i = "", e = this;
        if (parseFloat(t.value) > 0 && parseFloat(t.value) <= 1 && e.options.percent) i = e.formatPercent(t.value); else if (isNaN(parseFloat(t.value))) i = t.value; else {
            i = e.formatComma(t.value);
            var s = parseFloat(i);
            0 === s % 1 && (i = i.split(".")[0]), e.options.money && (i = "$" + i);
        }
        return i;
    }
    ...
    hoverOnDot: function(t, i, e) {
        var s = this, o = this.formatComma(t.value), n = 55 + this.x(t.date), a = 25 + this.y(t.value);
        d3.select(e).transition().attr("r", this.options.dotRadius + 3);
        var r = d3.select(".line")[0][0].getBBox().width;
        this.x(t.date) >= r && (n -= 55), d3.select(s.el).select(".hoverbox").style("right", "auto"), 
        d3.select(s.el).select(".hoverbox").style("left", n + "px"), d3.select(s.el).select(".hoverbox").style("top", a + "px"), 
    ...
    update: function(t) {
        ...
        var e = this.color.domain().map(function(e) {
            return {
                ...
                values: t.map(function(t) {
                    var s, o = t[i.options.x];
                    return s = null === t[e] ? null : +t[e], {
                        date: o,
                        value: s
                    };
                })
        ...
        if (this.options.dashed) {
            ...
            this.options.dashed.forEach(function(t) {
                var i = e[t.line], o = {};
                o.name = JSON.parse(JSON.stringify(i.name)), o.values = [], o.dashed = !0, t.span.forEach(function(t) {
        ...
        for (var o = 0; o < e.length; o++) {
            for (var n = [], a = 0; a < e[o].values.length; a++) {
                var r = e[o].values[a].value;
                isNaN(r) || null === r || n.push(e[o].values[a]);
            }
            e[o].values = n;
        }
        ...
        var h = this.y.domain();
        this.y.domain([ h[0] - .1 * h[0], h[1] + .1 * h[1] ]);
        var l = d3.select(this.el + " svg g");
        "auto" == i.options.xInterval ? this.xAxis.ticks(t.length) : this.xAxis.ticks(i.options.xInterval), 
        l.select(".y.axis").transition().call(this.yAxis), l.select(".x.axis").transition().call(this.xAxis);
        var d = function(t, i) {
            return 10 * i;
        }, p = l.selectAll(".line").data(e);
        p.transition().duration(500).delay(d).attr("d", function(t) {
            return i.line(t.values);
        }), p.enter().append("path").attr("class", "line").attr("d", function(t) {
            return i.line(t.values);
        ...
        }).style("stroke-opacity", i.options.opacity);
        for (var o = 0; o < e.length; o++) {
            var n = e[o].values, c = l.selectAll(".dotset" + o).data(n);
            c.transition().duration(500).delay(d).attr("data", function(t) {
                return t.value;
            }).attr("cx", function(t) {
                return i.x(t[i.options.x]);
            }).attr("cy", function(t) {
                return i.y(t.value);
            }), c.enter().append("circle").attr("class", "dot dotset" + o).attr("r", this.options.dotRadius).style("fill", function() {
                return i.color(e[o].name);
    ...
    update: function(t) {
        ...
        if (e.select("path").transition().style("fill", function(t) {
            ...
            if (d3.select(this).style("fill-opacity", 1), i.options.hover) {
                ...
                d3.select(i.el).select(".hoverbox").html(e + ": " + o + "%"), d3.select(i.el).select(".hoverbox").style("display", "block");
            }
    ...
    drawChart: function() {
        ...
        this.y = d3.scale.ordinal().rangeRoundBands([ 0, this.height ], .05, .5), this.xAxis = d3.svg.axis().scale(this.x).orient("top").tickSize(-1 * this.height, 0, 0).tickFormat(function(i) {
            return t.formatLarge(i);
    ...
    update: function(t) {
        ...
        var n = this.svg.select(".bars").selectAll(".bar").data(t);
        n.transition().attr("id", function(t) {
            return t[e];
        }).attr("x", function(t) {
            var e = i.x(Math.min(0, t[s]));
            return e >= i.x(0) && (e -= i.options.roundRadius), e;
        }).attr("y", function(t) {
            return i.y(t[s]);
        }).attr("width", function(t) {
            var e = Math.abs(i.x(t[s]) - i.x(0));
            return e += i.options.roundRadius;
        }).attr("height", i.y.rangeBand()).attr("rx", i.options.roundRadius).style("fill", function(t) {
            return i.color(t[e]);
        }).style("fill-opacity", function(t) {
            return t[e] == i.options.highlight ? 1 : i.options.opacity;
        }), n.enter().append("rect").attr("class", "bar").attr("id", function(t) {
            return t[e];
        }).attr("x", function(t) {
            var e = i.x(Math.min(0, t[s]));
            return e >= i.x(0) && (e -= i.options.roundRadius), e;
            ...
            d3.select(this).style("fill-opacity", 1), d3.select(i.el).select(".hoverbox").html(t[e] + ": " + (i.options.percent ? i.formatPercent(t[s]) : i.formatComma(t[s]))), 
            d3.select(i.el).select(".hoverbox").transition().style("display", "block");
        }).on("mouseout", function(t) {
            var s = i.options.opacity;
            t[e] == i.options.highlight && (s = 1), d3.select(this).style("fill-opacity", s), 
            d3.select(i.el).select(".hoverbox").transition().style("display", "none");
        }), n.exit().remove();
        var a = d3.rgb(d3.select(this.el).style("background-color")).toString(), r = this.svg.select(".bars").selectAll(".hider").data(t);
        r.transition().attr("width", 2 * i.options.roundRadius).attr("height", i.y.rangeBand()).attr("x", i.x(0)).attr("y", function(t) {
            return i.y(t[e]);
        }).attr("transform", function(t) {
            var e = i.x(Math.min(0, t[s]));
            return e < i.x(0) ? "translate(0, 0)" : "translate(" + -1 * 2 * i.options.roundRadius + ", 0)";
            ...
            return i.y(t[e]);
        }).style("fill-opacity", 1).style("fill", a).attr("transform", function(t) {
            var e = i.x(Math.min(0, t[s]));
            return e < i.x(0) ? "translate(0, 0)" : "translate(" + -1 * 2 * i.options.roundRadius + ", 0)";
    ...
    drawChart: function() {
        ...
        this.height = ("auto" === this.options.height || void 0 === this.options.height ? parseInt(d3.select(this.el).style("height")) : this.options.height) - this.margin.top - this.margin.bottom, 
        this.options.title && (this.height = this.height - 30), this.formatPercent = d3.format(".2%"), 
        this.formatPercentAxisLabel = d3.format("p"), this.formatMoney = d3.format("$"), 
        this.formatLarge = d3.format("s"), this.formatComma = d3.format(",.2f"), this.x = d3.scale.linear().range([ 10, this.width - this.options.yWidth ]).nice(), 
        this.y = d3.scale.ordinal().rangeRoundBands([ 0, this.height ], .05, 0), this.svg = d3.select(this.el).append("div").attr("class", "geodash barchart-html").style("width", this.width + this.margin.right + "px").style("margin-top", this.margin.top + "px").style("margin-bottom", this.margin.bottom + "px").style("margin-left", this.margin.left + "px"), 
        this.xAxisElement = this.svg.append("div").attr("class", "x axis").style("margin-left", this.options.yWidth + "px"), 
        this.yAxisElement = this.svg.append("div").attr("class", "y axis").style("padding-bottom", this.options.padding + "px").style("padding-top", this.options.padding + 15 + "px"), 
    ...
    update: function(t) {
        var i = this;
        this.data = t;
        var e = this.options.y, s = this.options.x;
        t.forEach(function(t) {
            null != t[s] && (t[s] = +t[s]);
        }), this.color = d3.scale.ordinal().range(this.options.barColors);
        var o = d3.extent(t, function(t) {
        ...
        var n = this.svg.select(".bars").selectAll(".bar").data(t);
        if (n.transition().attr("geodash-id", function(t) {
            return t[e];
            ...
            return i.options.barHeight + "px";
        }).style("opacity", function(t) {
            for (var s = 0; s < i.options.highlight.length; s++) if (i.options.highlight[s] == t[e]) return 1;
            return i.options.opacity;
        }).style("border-top-right-radius", function(t) {
            var e = 0, o = i.x(Math.min(0, t[s]));
            ...
            return i.color(t[e]);
        }), n.exit().remove(), this.options.drawX) {
            var a = this.x.ticks(i.options.numberTicks), r = this.xAxisElement.selectAll(".tick").data(a);
            r.transition().style("left", function(t) {
                return i.x(t) + "px";
            });
            var h = r.enter().append("div").attr("class", "tick").style("left", function(t) {
                return i.x(t) + "px";
            }).style("height", "100%");
            r.exit().remove(), h.append("div").attr("class", "gd-label").text(function(t) {
                var e = "";
                if (i.options.percent) {
                    e = i.formatPercentAxisLabel(t);
                    var s = parseFloat(e);
                    .001 > s % 1 && s % 1 > 0 && (s = s.toFixed(0), e = s + "%");
                } else e = i.formatLarge(t), i.options.money && (e = "$" + e);
                return e;
            }).style("margin-left", function() {
                var t = d3.select(this).style("width"), i = -1 * (parseInt(t) / 2);
                return i + "px";
            }), h.append("div").attr("class", "line").style("height", "100%");
        }
        if (this.options.drawY) {
            var l = this.y.domain(), d = this.yAxisElement.selectAll(".gd-label").data(l);
            d.transition().style("margin-top", function() {
                return i.options.padding + "px";
            }).style("height", function() {
                return i.options.barHeight + "px";
                ...
                var o = i.data[e][s], n = parseInt(d3.select(i.el).select(".bars").style("width"));
                Math.abs(i.x(o) - i.x(0));
                var a = i.x(Math.min(0, o)), r = n - a + 2;
                return r + "px";
            }).text(function(t) {
                return t;
            }), d.enter().append("div").attr("class", "gd-label").style("margin-top", function() {
                return i.options.padding + "px";
            }).style("height", function() {
                return i.options.barHeight + "px";
            }).style("line-height", function() {
                ...
                var o = t, n = i.data[e][s], a = d3.select(i.el).selectAll(".bar")[0][e];
                d3.select(a).style("opacity", .9);
                var r = "";
                if (null !== n) if (i.options.percent) r = i.formatPercent(n); else {
                    r = i.formatComma(n);
                    var h = parseFloat(r);
                    0 === h % 1 && (r = r.split(".")[0]), i.options.money && (r = "$" + r);
                } else r = "No Data";
                d3.select(i.el).select(".hoverbox").html(o + ": " + r), d3.select(i.el).select(".hoverbox").transition().style("display", "block");
            }).on("mouseout", function(t, s) {
                for (var o = i.options.opacity, n = 0; n < i.options.highlight.length; n++) i.options.highlight[n] == t[e] && (o = 1);
                var a = d3.select(i.el).selectAll(".bar")[0][s];
                d3.select(a).style("opacity", o), d3.select(i.el).select(".hoverbox").transition().style("display", "none");
            }), d.exit().remove();
        }
    },
    setColor: function(t) {
